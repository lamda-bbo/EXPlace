...
# # ----------------------------------------------------------------------------
  ...
  } elseif {[env_var_exists_and_non_empty TRITON_MACRO_PLACEMENT]} {
    set additional_tritonmp_args ""
    append additional_tritonmp_args " -halo {$halo_x $halo_y}"
    append additional_tritonmp_args " -channel {$channel_x $channel_y}"
    log_cmd global_placement
    log_cmd macro_placement {*}$additional_tritonmp_args
  } elseif {[env_var_exists_and_non_empty REPLACE_MACRO_PLACEMENT_PLACE]} {
    log_cmd global_placement -timing_driven
    log_cmd write_macro_placement $::env(RESULTS_DIR)/replace_mp_out
  } elseif {[env_var_exists_and_non_empty REPLACE_MACRO_PLACEMENT_EVAL]} {
    log_cmd source $::env(RESULTS_DIR)/replace_mp_out
  } else {
