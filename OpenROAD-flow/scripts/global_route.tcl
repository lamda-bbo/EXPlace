...
proc global_route_helper {} {
  ...
  if { !$::env(SKIP_INCREMENTAL_REPAIR) } {
    ...
    global_route -end_incremental -congestion_report_file $::env(REPORTS_DIR)/congestion_post_repair_design.rpt -allow_congestion
    ...
    set result [catch {detailed_placement} errMsg]
    # Route only the modified net by DPL
    global_route -end_incremental -congestion_report_file $::env(REPORTS_DIR)/congestion_post_repair_timing.rpt -allow_congestion
  }

  global_route -start_incremental -allow_congestion
  recover_power
  # Route the modified nets by rsz journal restore
  global_route -end_incremental -congestion_report_file $::env(REPORTS_DIR)/congestion_post_recover_power.rpt -allow_congestion
